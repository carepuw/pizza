...
import { 
    deletePizzaCart, 
    deleteAllFromPizzaCart, 
    changePizzaCountOnCart,
...
const Cart = () => {
    ...
    const pizzasObjectKeys = Object.keys(items).map( (key) => {
        return items[key]
    });

    const onDeletePizza = (obj, counter) => {
        let key = obj.id + '.' + obj.size + '.' + obj.types;
        dispatch(deletePizzaCart(key, obj, counter));
    }

    const onClickClearCart = () => {
        dispatch(deleteAllFromPizzaCart());
    }

    const changePizzaCounter = (counter, obj, bool) => {
        let key = obj.id + '.' + obj.size + '.' + obj.types;
        if (counter === 0) {
            counter = 1;
            dispatch(deletePizzaCart(key, obj, counter));
        } else {
            dispatch(changePizzaCountOnCart(key, obj, bool))
        }
    }

    const payNow = () => {
        let masPiz = Object.keys(items).map( (key) => {
            return {
                name: items[key][0].name,
                count: items[key].length,
                price: items[key][0].price * items[key].length,
            }
        })
        console.log('ВАШ ЗАКАЗ:');
        masPiz.map( (item) => {
            console.log('   ',`${item.name}, `,`Количество: ${item.count},`,`Стоимость: ${item.price}руб`)
            return 0
        })
        console.log(`ПОЛНАЯ СТОИМОСТЬ ЗАКАЗА: ${totalPrice} РУБ`)
    }
        ...
        <div className="container container--cart">
            ...
            <div className="cart">
                ...
                <div className="content__items">
                    {
                        pizzasObjectKeys.map( (item,index) => 
                            <CartPizza 
                                editPizzaCount={(count, obj, bool) => changePizzaCounter(count, obj, bool)}
                                deletePizza={(obj, counter) => onDeletePizza(obj, counter)}
                                key={index} 
                                obj={item[0]} 
                                counter={item.length}
                            />
